...
HtmlReporter.prototype.writeHead = function(testcase) {
    console.log('<br>');
    console.log('<span style="font-size: 1.2em">  ' + testcase.name + '</span>');
};
...
HtmlReporter.prototype.writeResult = function(result) {
    ...
    buf.push('    ');
    if (result.success) {
        buf.push('<span style="color:green">' + Symbols.OK + '</span> ' +
            '<span style="color:grey">' + result.name + '</span>');
    } else {
        buf.push('<span style="color:red">' + Symbols.NG + '</span> ' +
            '<span style="color:grey">' + result.name + '</span>');
    }
...
HtmlReporter.prototype.writeFinish = function(results) {
    ...
    if (results.success) {
        ...
        buf.push('<span style="color:red">' + Symbols.NG + ' ' + stats.failed +
...
HtmlReporter.prototype.phantomDebug = function(debug) {
    var buf = [];
    buf.push('  ');
    buf.push('<span style="color:cyan">' + Symbols.PDB + ' ' + debug + '</span>');
    console.log(buf.join(''));
};
