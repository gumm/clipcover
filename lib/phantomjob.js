 ...
 */
var log = new reportPlugin.Spec();

/**
 * A output file writer.
 * @type {reportPlugin.Tap}
 */
var write = new reportPlugin.Tap(fs, config.outputFile);
...
page.onCallback = function(obj) {
    ...
    switch (type) {
        ...
        case 'success':
        case 'failure':
            log.writeResult(data);
            write.writeResult(data);
            break;
        case 'debug':
            log.phantomDebug(data);
            write.phantomDebug(data);
            break;
        case 'finish':
            log.writeFinish(data);
            write.writeFinish(data);
            var reportResult = coverage.save(page);
            log.phantomDebug('Report result: ' + reportResult);
            phantom.exit(0);
            break;
        default:
...
page.open(config.testTarget, function(status) {
    if (status === 'success') {
        log.phantomDebug('Testpage Opened: ' + page.url);
    } else {
